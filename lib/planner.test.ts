...
import { MissionCoverageError, missionDurationLabel, planForMaxXpHorizon, planForTarget, summarizeCraftRows } from "./planner";
...
describe("planForMaxXpHorizon", () => {
  beforeEach(() => {
    mockedLoadLootData.mockReset();
    mockedSolveWithHighs.mockReset();
  });

  it("does not create craft variables for non-craftable base ingredients", async () => {
    ...
    await planForMaxXpHorizon(baseProfile(), 1);

    const boundedCraftVars = new Set(
      capturedModel
        .split("\n")
        .map((line) => line.match(/^\s*0 <= (c_\d+) <= /)?.[1])
        .filter((variable): variable is string => Boolean(variable))
    );
    const craftableRecipeCount = Object.values(recipes).filter(Boolean).length;
    expect(boundedCraftVars.size).toBe(craftableRecipeCount);
    expect(craftableRecipeCount).toBeLessThan(Object.keys(recipes).length);
  });

  it("can exclude stone fragments from mission drops", async () => {
    mockedLoadLootData.mockResolvedValue({
      missions: [
        {
          afxShip: 0,
              ...
              targets: [
                {
                  ...
                  targetAfxId: 10000,
                  items: [
                    { afxId: 47, afxLevel: 1, itemId: "soul-stone-1", counts: [1, 0, 0, 0] },
                    { afxId: 1, afxLevel: 1, itemId: "puzzle-cube-1", counts: [1, 0, 0, 0] },
                  ],
                },
              ],
            },
    ...
    const result = await planForMaxXpHorizon(profile, 1, {
      missionDropRarities: { fragments: false },
    });

    expect(result.missions[0].expectedYields.map((row) => row.itemId)).toEqual(["puzzle-cube-1"]);
  });

  it("adds per-duration wall-clock launch caps for XP horizons", async () => {
    mockedLoadLootData.mockResolvedValue({
      missions: [
        {
          afxShip: 0,
    ...
    await planForMaxXpHorizon(profile, 1);

    expect(capturedModel).toContain("dc_0: 1 m_0 <= 6");
    expect(capturedModel).toContain("ma_0: 1 m_0 - 1 ml_0_0 - 1 ml_0_1 - 1 ml_0_2 = 0");
    expect(capturedModel).toContain("lc_0: 34560 ml_0_0 <= 86400");
  });

  it("models natural same-ship level progression inside XP horizons", async () => {
    mockedLoadLootData.mockResolvedValue({
      missions: [
        {
          afxShip: 1,
              ...
              targets: [
                {
                  totalDrops: 1,
                  targetAfxId: 10000,
                  items: [{ afxId: 1, afxLevel: 1, itemId: "puzzle-cube-1", counts: [2, 0, 0, 0] }],
                },
              ],
            },
          ],
    ...
    const phasedModel = capturedModels.find((model) => model.includes("pa_")) || "";
    expect(phasedModel).toContain("mx_");
    expect(phasedModel).toContain("pa_");
    expect(phasedModel).toContain("pb_");
    expect(phasedModel).toContain("Binary");
  });
});
